...
RTCPeerConnection.prototype.addIceCandidate = function addIceCandidate(
  ...
  if (!candidate?.candidate) {
    // The WebRTC specification says:
    // * if the `candidate` is undefined/missing
    // * if the `candidate` parameter is null
    // * if the `candidate` parameter has an undefined/missing `candidate` property
    // * if the `candidate` parameter has a `candidate: ""` property
    // Then, the message is treated as end-of-candidates.
    // This code normalizes all those conditions into a single value, for convenience.
...
RTCPeerConnection.prototype.setLocalDescription = function setLocalDescription(
  ...
  // Support parameterless setLocalDescription() per spec.
  // The type is inferred from signalingState, and a fresh offer/answer
  // is created automatically if no description is provided.
  if (
    !arguments.length ||
    (description && !description.type && !description.sdp)
  ) {
    var self = this;
    var state = this.signalingState;
    var promise;
    if (state === "closed") {
      ...
      promise = self.createAnswer().then(function (desc) {
        return self._pc.setLocalDescription(desc);
      });
    } else {
      promise = self.createOffer().then(function (desc) {
        return self._pc.setLocalDescription(desc);
      });
    }
    if (arguments.length >= 3) {
